...
function fig = PlotAssignments_RPO_NoPath(CBBA_Params,CBBA_Assignments, agents, tasks, simDurWks, figureID)
...
for m=1:length(tasks)
    %%plot3(tasks(m).INCL + [0 0], tasks(m).RAAN + [0 0], [tasks(m).start tasks(m).end],'x:','color','k','LineWidth',3);
    %%plot3(tasks(m).INCL , tasks(m).RAAN , tasks(m).start ,'x','color','k','LineWidth',3);
    plot3(tasks(m).INCL , tasks(m).RAAN , 0 ,'x','Color', [100, 100, 100] / 255,'LineWidth',3);
    hold on;
    %text(tasks(m).INCL+offset, tasks(m).RAAN+offset, tasks(m).start, ['RSO' num2str(m)]);
    %%text(tasks(m).INCL+offset, tasks(m).RAAN+offset, 0, ['RSO' num2str(m)]);
...
hold off;
title(['Servicing Fulfillment with RAAN Drift'])
