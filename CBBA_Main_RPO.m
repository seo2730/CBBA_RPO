...
function [CBBA_Data, Total_Score] = CBBA_Main_RPO(CBBA_Params, agents, tasks, Graph)

% Initialize CBBA parameters
% CBBA_Params = CBBA_Init_RPO(length(agents),length(tasks));
% CBBA_Params.JD = JD;
...
while(doneFlag == 0)
    ...
    for n=1:CBBA_Params.N
        ...
        [CBBA_Data(n), newBid] = CBBA_Bundle_RPO(CBBA_Params, CBBA_Data(n), agents(n), tasks);
