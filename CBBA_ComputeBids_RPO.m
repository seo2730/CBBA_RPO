...
function [CBBA_Data, bestIdxs, taskTimes, feasibility] = CBBA_ComputeBids_RPO(CBBA_Params, CBBA_Data, agent, tasks, feasibility)
        ...
        if( isempty(find(CBBA_Data.path(1,1:L(1,1)-1) == m)) )
            ...
            for j=1:L(1,1)
                if( feasibility(m,j) == 1 )
                    ...
                    %%%%%%%%%%%% Add in function to return a best time as
                    %%%%%%%%%%%% well as min/max times
%                     disp(m)
%                     disp(j)
                    %[score, minStart, maxStart] = Scoring_CalcScore_RPO(CBBA_Params,agent,tasks(m),taskPrev,timePrev,taskNext,timeNext, CBBA_Data, tasks, j);
                    [score, minStart, maxStart] = Scoring_CalcScore_RPO_best(CBBA_Params,agent,tasks(m),taskPrev,timePrev,taskNext,timeNext, CBBA_Data, tasks, j);
                    %[score, minStart, maxStart] = Scoring_CalcScore_RPO_best_global(CBBA_Params,agent,tasks(m),taskPrev,timePrev,taskNext,timeNext, CBBA_Data, tasks, j);
                    ...
                    if(~skip)
                        ...
                        if(score > bestBid )
                            ...
                            %!!! Changed to bestStart from minStart
                            bestTime  = minStart;  % Select "bestStart" start time as optimal
                        end
                    end
                end
            end
